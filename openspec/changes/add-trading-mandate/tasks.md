...
- [x] 1.4 Include mandate (`strip()`-normalized, hashed) in `_run_signature()` so a changed mandate invalidates a resumable checkpoint; unchanged signature when mandate is empty.
...
- [x] 2.2 Append the mandate block to the `create_msg_delete` placeholder message.
- [x] 2.3 Unit tests: empty-state no-op, formatted block content, placeholder inclusion.
